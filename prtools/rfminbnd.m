function [xf,fval,exitflag,output] = rfminbnd(funfcn,ax,bx,options,varargin)
%FMINBND Single-variable bounded nonlinear function minimisation.
%   X = FMINBND(FUN,x1,x2) attempts to find  a local minimiser X of the function 
...
%
%   X = FMINBND(FUN,x1,x2,OPTIONS) minimises with the default optimisation
...
starttime = clock;
runtime = 0;
while ( abs(xf-xm) > (tol2 - 0.5*(b-a)) ) && runtime < prtime
    gs = 1;
