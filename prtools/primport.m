...
function out = primport(a)
	...
	if isstruct(a)
		...
		if isfield(struct(a),'d') & isfield(struct(a),'l') & ...
			...
			prwarning(3,'prtools 3.x dataset converted to 4.x');
		...
		elseif isfield(struct(a),'data') & isfield(struct(a),'lablist') & ...
			...
			prwarning(3,'prtools 4.x converted from structure');
		
		else
			out = [];
			prwarning(1,'unknown format! The structure cannot be converted into a dataset');
		end
