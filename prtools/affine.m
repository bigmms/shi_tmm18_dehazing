...
% It defines a mapping W based on a linear transformation R and an offset. 
...
function w = affine(R,offset,lablist_in, lablist_out,size_in,size_out)
  ...
  if (nargin == 1) | (~isa(offset,'prmapping'))
		...
		[m,k] = size(R);
		if (nargin < 6)
			prwarning(3,'SIZE_OUT is not specified. The number of columns of R, %d, is assumed.', k); 
			size_out = k; 
		end
		if (nargin < 5)
			prwarning(3,'SIZE_IN is not specified. The number of rows of R, %d, is assumed.', m); 
			size_in = m; 
		end
		if (nargin < 4)
			prwarning(3,'LABLIST_OUT is not specified, [1:%d]'' assumed.', k); 
			lablist_out = []; 
		end
		if (nargin < 3) 
			prwarning(3,'LABLIST_IN is not specified, [1:%d]'' assumed.', m); 
			lablist_in = []; 
		end
	...
	elseif isa(R,'prmapping') 
        ...
        elseif size(w1,1) == 1
          ...
          % interpet d1.rot as a diagonal
          % don't use diag, too slow
