%NU_SVRO Support Vector Optimiser
%
%   [V,J] = NU_SVRO(K,Y,C)
%
% INPUT
...
function [v,J,epsilon_or_nu] = nu_svro(K,y,C,svr_type,nu_or_epsilon,pd,abort_on_error)
  ...
  lb = zeros(2*n,1);
    ...
    % Make the kernel matrix K positive definite.
  	...
  	if (i > -30),
	  	prwarning(3,'K is not positive definite. The diagonal is regularised by 10.0^(%d)*I',i);
  	end
  	i = i+2;
  	D = D + (10.0^(i)) * eye(n);
  end
  ...
  if isempty(v) 
    ErrMsg = 'Quadratic Optimisation failed.';
  ...
  case 'nu'
    ...
    if isempty(Ip) | isempty(Im);
      prwarning(3,'epsilon and b are not unique: values from the admissible range will be taken');
      J0 = find(av <= vmin); % non-sv's
      if ~isempty(J0)
        y_minus_wx_0 = y(J0)-K(J0,J)*v(J); 
      else
        prwarning(3,'There are no non-sv''s: admissible (eps,b) range is partially unbounded');
      end
    end
  ...
  case 'epsilon'
    if ~isempty(I)      
      ...
      prwarning(3,'b is not unique: value from the admissible range will be taken');
      ...
      if ~isempty(J0)
        ...
        prwarning(3,'Thers are no non-sv''s');
      end
...
function [v,J, epsilon_or_nu] = ErrHandler(K,y,ErrMsg,abort_on_error)
  if abort_on_error
    ...
    prwarning(2,[ErrMsg 'Pseudoinverse Regression is computed instead.']);
    n = size(K,1);
